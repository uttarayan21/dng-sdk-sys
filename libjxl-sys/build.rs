use std::path::*;
pub fn main() {
    let source = std::env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR not set");
    let source = Path::new(&source);
    let mut vendor = source.join("vendor");
    if !vendor.exists() {
        // Read from env
        vendor = PathBuf::from(std::env::var("LIBJXL_DIR").expect("VENDOR not set"));
    }
}

const JPEGXL_INTERNAL_SOURCES_DEC: [&str; 214] = [
    "jxl/ac_context.h",
    "jxl/ac_strategy.cc",
    "jxl/ac_strategy.h",
    "jxl/alpha.cc",
...
const JPEGXL_INTERNAL_SOURCES_ENC: [&str; 107] = [
